...
struct MapView<T: Equatable, Content: View>: UIViewRepresentable {
    var districtState: DistrictState = .all
    var annotationItems: [T] = []
    var overlays: [MKOverlay] = []
    var settings: MapSettings = MapSettings()
    var region: MKCoordinateRegion? = nil
    var route: MKRoute? = nil
    var eventAnnotation: MKPointAnnotation? = nil

    @ViewBuilder var content: (T) -> MapMarkerView<Content>
    
    func makeCoordinator() -> MapCoordinator<T, Content> {
        MapCoordinator()
    }
    ...
    func updateUIView(_ view: MKMapView, context: Context) {
        ...
        if annotationItems != context.coordinator.annotationItems {
            let annotations = annotationItems.map { annotationItem in
                return MapAnnotation(content: content, annotationItem: annotationItem)
            }
        ...
        if let polyline: MKPolyline = route?.polyline {
        ...
        if let eventAnnotation = eventAnnotation {
            view.addAnnotation(eventAnnotation)
        }
...
struct EmptyAnnotationValue: Equatable & Identifiable {
    var id: String = UUID().uuidString
}

extension MapView where Content == EmptyView, T == EmptyAnnotationValue {
    init(
        ...
        self.region = region
        self.content = { _ in MapMarkerView()}
...
extension MapView {
    init(
        ...
        content: @escaping (T) -> MapMarkerView<Content>
    ) {
